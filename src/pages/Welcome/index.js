import React from 'react';
import { View, Text, StyleSheet, SafeAreaView, Image, TouchableOpacity, Linking } from 'react-native';

// import { Container } from './styles';
const imageProfile  = 'https://avatars.githubusercontent.com/u/33238548?s=400&u=fe9c83aaf2785f2c3708a4a777ba676864de2775&v=4';
const iconeGithub  = 'data:image/png;base64,';
const imageButton = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSs4G0v6XjTYrdUJhPREoIOd0Ue0MVq_Q4apA&usqp=CAU';
const Welcome = () => {
  return(
    <SafeAreaView style={style.container}>
      <View style={style.content}>
        <Image 
          accessibilityLabel='Marcos com fundo azul'
          style={style.icone} 
          source={{uri: iconeGithub}}
        />
        <Image 
          accessibilityLabel='Marcos com fundo azul'
          style={style.avatar} 
          source={{uri: imageProfile}}
        />
        <Text 
          accessibilityLabel='Nome Marcos: Neves'
          style={style.text}>Marcos Neves</Text>
        <Text 
          accessibilityLabel='NickNAme: Marcos Neves'
          style={[style.text, style.textNikeName]}>nevesmarcos42</Text>
        <Text 
          accessibilityLabel='Description: Aluno de Graduação 
          em Ciência da Computação | Formação 
          técnica Programação de Jogos Digitais.'
          style={[style.text, style.textDescription]}>Aluno de Graduação 
          em Ciência da Computação | Formação 
          técnica Programação de Jogos Digitais.
        </Text>
        <TouchableOpacity
          activeOpacity={0.7}
          onPress={clickHandler}
          style={style.touchableOpacityStyle}>
          <Image
            source={{uri: imageButton}}
            style={style.floatingButtonStyle}
          />
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
  
}

export default Welcome;

const clickHandler = async () => {
  const res = await Linking.canOpenURL('https://github.com/nevesmarcos42');
  if(res){
    await Linking.openURL('https://github.com/nevesmarcos42')
  }
};
const style = StyleSheet.create({
  container: {
    backgroundColor: 'black',
    flex : 1, //Expande para a tela inteira
    justifyContent: 'center',
  },
  content: {
    alignItems: 'center'
  },
  text: {
    fontSize: 40,
    fontWeight: 'bold',
    color: '#ffffff',
  },
  textNikeName: {
    fontSize: 20,
    fontWeight:'normal',
  },
  textDescription: {
    fontSize: 17,
    padding: 15,
    fontWeight:'normal',
  },
  avatar : {
    height: 150,
    width: 150,
    borderRadius: 75,
    borderColor: 'white',
    borderWidth: 1,
  },
  icone : {
    height: 60,
    width: 60,
    borderRadius: 30,
    borderColor: 'white',
    borderWidth: 1,
    top: -30,
  },
  touchableOpacityStyle: {
    width: 50,
    height: 50,
    alignItems: 'center',
    justifyContent: 'center',
    bottom: -20,
  },
  floatingButtonStyle: {
    resizeMode: 'contain',
    width: 80,
    height: 80,
    borderColor: 'gray',
    borderWidth: 2, 
    borderRadius: 20,
  },
});